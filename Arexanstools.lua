...
task.spawn(function()
    ...
    -- ====================================================================
    -- == BAGIAN MODUL LOADER (REVISI)                                 ==
    -- ====================================================================
    local RECORDING_TAB_URL = "https://raw.githubusercontent.com/AREXANS/ArtexTool/main/recording_tab.lua"
    local SETTINGS_TAB_URL = "https://raw.githubusercontent.com/AREXANS/ArtexTool/main/settings_tab.lua"
    
    local dynamicTabs = {}

    local function loadModule(url, dependencies)
        local success, moduleWrapper = pcall(function()
            return loadstring(game:HttpGet(url))()
        end)
        if success and type(moduleWrapper) == "function" then
            local module = moduleWrapper(dependencies)
            if type(module) == "table" and type(module.setup) == "function" then
                return module
            else
                warn("Modul dari", url, "tidak mengembalikan tabel dengan fungsi 'setup'.")
            end
        else
            warn("Gagal memuat atau menjalankan pembungkus modul dari:", url, "| Error:", moduleWrapper)
        end
        return nil
    end
    ...
    local sharedDependencies = {
        ...
        showGenericRenamePrompt = showGenericRenamePrompt,
        showRecordingFilePicker = showRecordingFilePicker,
        -- Services & Globals
        ...
        -- Core Functions
        saveFeatureStates = saveFeatureStates,
        saveGuiPositions = saveGuiPositions,
        ConnectEvent = ConnectEvent,
        -- Settings & State
        Settings = Settings,
        isMiniToggleDraggable = isMiniToggleDraggable,
        -- Recording Specific
    ...
    -- Wrapper untuk switchTab yang menangani tab statis dan dinamis
    local originalSwitchTab = switchTab
    switchTab = function(tabName)
        -- Sembunyikan semua tab (statis dan dinamis) terlebih dahulu
        PlayerTabContent.Visible = false
        GeneralTabContent.Visible = false
        TeleportTabContent.Visible = false
        VipTabContent.Visible = false
        for _, tabData in pairs(dynamicTabs) do
            if tabData.content then tabData.content.Visible = false end
        end
        ...
        if tabName == "Player" then
            ...
            updatePlayerList()
        elseif tabName == "Umum" then
            GeneralTabContent.Visible = true
        elseif tabName == "Teleport" then
            TeleportTabContent.Visible = true
        elseif tabName == "VIP" then
            VipTabContent.Visible = true
        elseif dynamicTabs[tabName] and dynamicTabs[tabName].content then
            dynamicTabs[tabName].content.Visible = true
        end
    end
    sharedDependencies.switchTab = switchTab


    -- Muat dan inisialisasi modul
    local function setupDynamicTab(name, url, dependencies)
        task.spawn(function()
            local module = loadModule(url, dependencies)
            if module then
                local tabButton = createTabButton(name, TabsFrame)
                local tabContent = module.setup() -- Panggil setup dari modul
                dynamicTabs[name] = {button = tabButton, content = tabContent}
                
                -- Pastikan konten tab baru tidak terlihat pada awalnya
    ...
    setupDynamicTab("Rekaman", RECORDING_TAB_URL, sharedDependencies)
    setupDynamicTab("Pengaturan", SETTINGS_TAB_URL, sharedDependencies)
